...
def main():
    ...
    page = st.sidebar.radio("Навигация", ["Дашборд", "Журнал операций", "Добавить операцию"])
    ...
    date_range = st.sidebar.selectbox(
        "Выберите период",
        ["Сегодня", "Текущая неделя", "Текущий месяц", "Произвольный период"]
    )
    ...
    if page == "Дашборд":
        ...
        if df.empty:
            ...
            # === 5️⃣ Доход по предметам ===
            st.subheader("Доход по подкатегориям")
            income_df = df[df['operation_type'] == 'доход']
            if not income_df.empty:
                income_by_subcat = income_df.groupby('subcategory')['amount'].sum().reset_index().sort_values(
                    'amount', ascending=False)
                income_chart = px.bar(
                    income_by_subcat.head(10),
                    x='subcategory',
                    y='amount',
                    text='amount',
                    title="Топ-10 подкатегорий по доходам"
                )
    ...
    # Журнал операций
        ...
        if df.empty:
            ...
            df_view = df[[
                ...
                'group_name', 'subgroup', 'amount', 'comment', 'created_at'
            ]]
            df_view.columns = [
                'Дата', 'Тип', 'Категория', 'Подкатегория',
                'Группа', 'Подгруппа', 'Сумма', 'Комментарий', 'Создано'
            ]
